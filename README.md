...
![Observability pipeline — JSONL transcripts flow into Postgres, then into Grafana; a separate Prometheus exporter feeds ccusage daily totals into the same dashboard](docs/images/pixel-demo/04-observability-pipeline.jpeg)

*Flow (left → right): JSONL transcripts (stack on the left) → `claude-pg-sync.py` → Postgres → Grafana dashboard (panel on the right). Below: `claude-exporter.py` ships `ccusage` daily totals via Prometheus into the same dashboard. Generated by `pixel-designer` via `nano-banana`.*
